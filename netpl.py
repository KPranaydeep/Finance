...
if "df" in st.session_state:
    ...
    # treat exact 0 as "transparent" for the heatmap
    daily_pnl = daily_pnl.replace(0, np.nan)

    # --- Calendar Heatmap (guarded normalization) ---
    with st.expander("📆 Calendar Heatmap of Daily P&L", expanded=True):
        if daily_pnl.dropna().empty:
            ...
            # guard: all NaN or max_abs==0 -> fall back to small epsilon to avoid divide-by-zero
    ...
    if goal_amount and goal_deadline:
        ...
        # Optional: download current cleaned data
        with BytesIO() as buf:
            df.to_excel(buf, index=False, engine="openpyxl")
            buf.seek(0)
            st.download_button("⬇️ Download Cleaned Trade Data", data=buf.getvalue(), file_name="cleaned_trades.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
