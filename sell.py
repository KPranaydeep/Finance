...
if uploaded_mmi_file:
    ...
    today_nifty = st.number_input("Today's Nifty", min_value=0.0, step=1.0)
    ...
    for date in future_dates:
        ...
        m3.append(m3_val); m3.pop(0)
        m5.append(m5_val); m5.pop(0)

    future_df['Predicted_MMI'] = predictions
    residual_std = np.std(y - model.predict(X))

    lowest_mmi_date = future_df['Predicted_MMI'].idxmin()
    lowest_mmi = future_df['Predicted_MMI'].min()
    ci_low = (lowest_mmi - 1.96 * residual_std, lowest_mmi + 1.96 * residual_std)

    highest_mmi_date = future_df['Predicted_MMI'].idxmax()
    highest_mmi = future_df['Predicted_MMI'].max()
    ci_high = (highest_mmi - 1.96 * residual_std, highest_mmi + 1.96 * residual_std)

    st.markdown("### 🔮 Forecast Summary (Next 120 Days)")
    st.write(f"🔻 **Lowest MMI:** {lowest_mmi:.2f} on {lowest_mmi_date.strftime('%d %b %Y')} (CI: {ci_low[0]:.2f}–{ci_low[1]:.2f})")
    st.write(f"🔺 **Highest MMI:** {highest_mmi:.2f} on {highest_mmi_date.strftime('%d %b %Y')} (CI: {ci_high[0]:.2f}–{ci_high[1]:.2f})")

    # Recommendations
    st.markdown("### 💡 MMI-Based Trading Recommendations")
    if lowest_mmi < 50:
        st.success(f"📥 **BUY on {lowest_mmi_date.strftime('%d %b %Y')}** – Forecast MMI {lowest_mmi:.2f} < 50")
    else:
        st.warning("❌ No BUY signal – forecast MMI stays above 50")

    if highest_mmi > 50:
        st.success(f"📤 **SELL on {highest_mmi_date.strftime('%d %b %Y')}** – Forecast MMI {highest_mmi:.2f} > 50")
    else:
