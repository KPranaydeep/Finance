...
def build_assessment(baseline, histories, as_of, future, policy, prior, latest_weights, now, comparisons=True):
    ...
    try:
        exit_comparisons = compare_exits(baseline, prices, as_of, policy, metrics) if comparisons else []
    except ValueError:
        # Exit comparison is supplementary evidence; it must not invalidate
